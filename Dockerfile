...
# The CLI is invoked as node_modules/prisma/build/index.js, not through the
# node_modules/.bin symlink: COPY dereferences symlinks, which would strand the
# entrypoint away from the .wasm files it loads relative to its own directory.
